...
const getMergedData = (history = []) => {
  ...
  history.forEach(entry => {
    ...
    } else if (entry.type === 'ucla') {
      dataPoint.uclaScore = entry.score;
    } else if (entry.type === 'sidas') {
      dataPoint.sidasScore = entry.score;
    } else if (entry.type === 'bcope') {
      // BCOPE score contains categoryTotals
      if (entry.score && entry.score.categoryTotals) {
        dataPoint.problemFocused = entry.score.categoryTotals.problemFocused;
        dataPoint.emotionFocused = entry.score.categoryTotals.emotionFocused;
        dataPoint.avoidant = entry.score.categoryTotals.avoidant;
      }
    }
...
const AnalyticsDashboard = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        if (token) {
          ...
          combinedHistory = [...combinedHistory, ...formatted];
        } else {
          const saved = localStorage.getItem('mh_assessment_history');
          if (saved) {
            combinedHistory = [...combinedHistory, ...JSON.parse(saved)];
          }
        }
      } catch (err) {
        console.error('Failed to fetch self assessments:', err);
      }

      // Fetch family-assessments
      try {
        const mockPatientId = '64b1f4c5e4b0f9a7d8c9e0a1'; // Hardcoded for MVP as per AssessmentFlow
        const res = await axios.get(`${API_URL}/api/family-assessments/patient/${mockPatientId}`);
        const formatted = res.data.map(a => ({
          type: a.scaleType,
          score: a.score,
          date: a.createdAt,
        ...
        console.error('Failed to fetch family assessments:', err);
      }
  ...
  const metricsConfig = {
    ...
    ucla: [
      { key: "uclaScore", label: "Loneliness Score", color: "#3b82f6" }
    ],
    sidas: [
      { key: "sidasScore", label: "Ideation Score", color: "#ef4444" }
    ],
    bcope: [
      { key: "problemFocused", label: "Problem-Focused", color: "#10b981" },
      { key: "emotionFocused", label: "Emotion-Focused", color: "#f59e0b" },
      { key: "avoidant", label: "Avoidant", color: "#ef4444" }
    ]
  };

  const activeMetrics = metricsConfig[activeTab];

  // Dynamically set Y-Axis domain based on the active tab
  const yAxisDomain = activeTab === 'sidas' ? [0, 50] : (activeTab === 'bcope' ? [0, 40] : [0, 10]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 sm:gap-8 mb-8 sm:mb-12">
          <div className="space-y-3 sm:space-y-4">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-base sm:text-lg font-medium max-w-2xl leading-relaxed">
              Precision longitudinal tracking of emotional markers using standardized clinical scales (GAD-7, GDS-15, UCLA-3, SIDAS-M, Brief COPE).
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          ...
          <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-[2rem] sm:rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
            <div className="p-4 sm:p-8 border-b border-slate-50 flex flex-wrap items-center justify-between gap-4">
              <div className="flex w-full md:w-auto overflow-x-auto gap-2 p-1.5 bg-slate-100 dark:bg-slate-800 rounded-2xl [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                {[
                  { id: "depression", label: "Depression" },
                  { id: "anxiety", label: "Anxiety" },
                  { id: "ucla", label: "Loneliness" },
                  { id: "sidas", label: "Ideation" },
                  { id: "bcope", label: "Coping" }
                ].map(t => (
                  <button key={t.id} onClick={() => setActiveTab(t.id)} className={`flex-1 sm:flex-none px-4 sm:px-6 py-3 sm:py-3.5 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === t.id ? 'bg-white dark:bg-slate-700 text-primary shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>
                    {t.label}
                  </button>
                ))}
              </div>
            ...
            <div className="p-2 sm:p-6 h-[400px] sm:h-[550px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={currentData} margin={{ top: 20, right: 10, left: -20, bottom: 20 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                  <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 10, fontWeight: 800 }} dy={15} minTickGap={15} />
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 10, fontWeight: 800 }} domain={yAxisDomain} dx={-10} />
                  <Tooltip contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 20px 50px rgba(0,0,0,0.1)', padding: '12px', fontSize: '12px' }} />
                  <Legend verticalAlign="top" height={80} iconType="circle" wrapperStyle={{ fontWeight: 800, fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.05em', paddingBottom: '20px' }} />
                  {activeMetrics.map(m => (
                    <Line
                      key={m.key}
                      type="monotone"
            ...
            <div className="bg-primary rounded-[2rem] sm:rounded-[3rem] p-8 sm:p-10 text-white shadow-2xl relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <p className="text-white/70 text-sm sm:text-base font-medium mb-8 sm:mb-10 leading-relaxed">
                  Your data demonstrate high fidelity across all monitoring markers including family care scales.
                </p>
              ...
              <div className="space-y-4 sm:space-y-6">
                {history.slice(-5).reverse().map((h, i) => (
                  <div key={i} className="flex items-center justify-between p-4 sm:p-6 bg-slate-50 dark:bg-slate-800 rounded-[1.25rem] sm:rounded-[1.5rem] border border-slate-100 dark:border-slate-700">
                    <div>
                      <p className="text-slate-900 dark:text-white font-black text-xs sm:text-sm capitalize">{h.type === 'ucla' ? 'Loneliness' : h.type === 'sidas' ? 'Ideation' : h.type === 'bcope' ? 'Coping' : h.type} Score</p>
                    ...
                    <span className="font-black text-primary text-base sm:text-lg">
                      {typeof h.score === 'object' ? 'Profile' : h.score}
                    </span>
                  </div>
                ))}
